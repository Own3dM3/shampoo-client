...
public class InteractionUtil implements Util {
    ...
    public static void placeblock(BlockPos pos, boolean packet) {
        BlockHitResult bhop = new BlockHitResult(Vec3d.of(pos), calcSide(pos),pos, true);
        if (packet) {
            mc.player.networkHandler.sendPacket(new PlayerInteractBlockC2SPacket(Hand.MAIN_HAND, bhop, 0));
        } else {
            mc.interactionManager.interactBlock(mc.player, Hand.MAIN_HAND, bhop);
        }
    }
}
