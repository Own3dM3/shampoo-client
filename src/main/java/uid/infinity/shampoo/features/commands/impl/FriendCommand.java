...
public class FriendCommand
    ...
    public void execute(String[] commands) {
        if (commands.length == 1) {
            if (shampoo.friendManager.getFriends().isEmpty()) {
                FriendCommand.sendMessage("Friend list empty D:.");
            } else {
                StringBuilder f = new StringBuilder("Friends: ");
                for (String friend : shampoo.friendManager.getFriends()) {
                    try {
        ...
        if (commands.length == 2) {
            if (commands[0].equals("reset")) {
                shampoo.friendManager.getFriends().clear();
                FriendCommand.sendMessage("Friends got reset.");
                return;
            }
            FriendCommand.sendMessage(commands[0] + (shampoo.friendManager.isFriend(commands[0]) ? " is friended." : " isn't friended."));
            return;
        }
        if (commands.length >= 2) {
            switch (commands[0]) {
                case "add" -> {
                    shampoo.friendManager.addFriend(commands[1]);
                    FriendCommand.sendMessage(Formatting.GREEN + commands[1] + " has been friended");
                    return;
                }
                case "del", "remove" -> {
                    shampoo.friendManager.removeFriend(commands[1]);
